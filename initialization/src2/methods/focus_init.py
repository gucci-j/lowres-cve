...
def instantiate_model_by_focus(
    ...
    for token in overlapping_tokens:
        target_embeddings[target_token_to_idx[token]] = source_embeddings[source_token_to_idx[token]]
        if not tie_word_embeddings:
            target_head_embeddings[target_token_to_idx[token]] = source_head_embeddings[source_token_to_idx[token]]
